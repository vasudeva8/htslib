...
static int updatedepth(rc_t *c, ce_t *e, int chk)
{
    ...
    if (chk) {  //check depth
        //FILE *fpy = fopen("/tmp/dpchk.csv", "a");
        //fprintf(fpy,"%lld,%lld:,", e->ord, c->w_st+off);
        for (i = 0; i < e->r->core.n_cigar; ++i) {
            ...
            for (j = 0; j < clen; ++j) {
                ...
                //fprintf(fpy,"%lld,%d,", c->w_st+off+j, dpth[off + j]+1);
            }
            off += clen;
        ...
        req = 1;
        // FILE *fpx = fopen("/tmp/dp1.csv", "a");
        // fprintf(fpx,"%lld,%lld:,", e->ord, c->w_st+off);
        for (i = 0; i < e->r->core.n_cigar; ++i) {
            if (!(bam_cigar_type(bam_cigar_op(cgr[i])) & 2)) {   //not consuming ref
                continue;
            }
...
int addtoreadcache(rc_t *c, ce_t *e, int *sts)
{
    ...
    if (c->w_en < e->r->core.pos) {  //post window, p:rocess and advance
        LG("wnd full\n");
        c->trgr = 2; //wnd full, go for processing
    } else if (c->tid != e->r->core.tid && c->tid != -2) {
        LG("tid change\n");
...
{
    int paired = (e->r->core.flag & BAM_FPAIRED) && !(e->r->core.flag & BAM_FUNMAP) && !(e->r->core.flag & BAM_FMUNMAP) && e->r->core.mtid != -1 && e->r->core.mpos != -1;
...
{//printf("reset\n");
    ...
    //else c->head_nsel->prev = NULL;
    c->tail_nsel = NULL;
}

/*static void clearcache(rc_t *c)
{
    khiter_t iter;
    ce_t *en = NULL;
    c->tid = -1;
    ...
    while (c->head_sel) {
        en = c->head_sel->next;
        c->selcnt--;
        retcache(c, c->head_sel);
        c->head_sel = en;
    }
    c->head_sel = c->tail_sel = NULL;
    while (c->head_ins) {
        en = c->head_ins->next;
        c->inscnt--;
        retcache(c, c->head_ins);
        c->head_ins = en;
    }
    c->head_ins = c->tail_ins = NULL;
    //todo do asserts
}
*/
...
int getfromreadcache_iter(void *p, void *s, int *tid, hts_pos_t *beg, hts_pos_t* end) //?
{
    ...
    *beg = b->core.pos;
    ...
    LG("gfrci: t %"PRIu64" s %"PRIu64" i %"PRIu64" n %"PRIu64"\n", c->rcnt, c->selcnt,c->inscnt, c->nselcnt);
    if (!ret) {
        assert(!c->inscnt && !c->selcnt);
        assert(!c->head_sel && !c->tail_sel);
        assert(!c->head_ins && !c->tail_ins);
...
void notifyend_iter(void *p, void *e)
{
    rc_t *c = (rc_t*)p;
    c->trgr = 4;//end
    c->tid = -3;//reset that it doesn't match to any
    retcache(c, (ce_t*)e);
}
